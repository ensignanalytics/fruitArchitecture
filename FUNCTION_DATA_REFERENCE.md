...
|---|---|---|
| `fruitArchitectureFromDEG()` | Reconstruct an architecture from a completed DEG table and annotation | `fruit_architecture` object |
| `fruitArchitecture()` | Run DESeq2 from raw counts, then reconstruct the architecture | `fruit_architecture` object containing DESeq2 results |
| `fruitArchitectureExampleData()` | Load all bundled synthetic example objects | Named list |
| `fruitArchitectureExample()` | Run and optionally export the complete bundled example | Invisible `fruit_architecture` object |
| `supportedSpecies()` | List planned species-registry entries and built-in annotation status | Data frame |
| `plot()` | Draw module, interface, or null-distribution figures | `ggplot` object |
| `summary()` | Return a compact structured summary | `summary.fruit_architecture` list |
| `print()` | Print the main analysis results | Invisibly returns the input object |
| `exportArchitecture()` | Export result tables and the full RDS object | Output directory, invisibly |
| `exportFigures()` | Export standard PDF, PNG, and TIFF figures | Generated file paths, invisibly |
...
completed. It is also the recommended entry point when integrating results from
methods other than DESeq2.

### Required data

`deg_table` must have one unique row per gene and contain the columns named by
`gene_id_col`, `log2fc_col`, and `padj_col`.

`annotation` must be long format and contain the same gene-identifier column
plus either `module` or `pathway_name`.
...
- `min_interface_genes`: strict overlap genes required for interface presence;
...
This function requires the suggested Bioconductor package `DESeq2`. It fits the
DESeq2 model, constructs a standardized DEG table, and calls
`fruitArchitectureFromDEG()`.
...
The data are synthetic and intended only for installation testing, examples,
...
Runs the DEG-table example. With `export = TRUE`, it writes tables, an RDS
object, and all standard figures.
...
`built_in_annotation` values are `FALSE`; users must supply `annotation`.
...
Reports species, architecture name, class, weighted ISI, Level 3A and Level 3B
status, robustness label, entropy, balance, expected Level 3B, observed Level
3B, and empirical P-value.
...
Returns a list containing metadata, primary metrics, null-model summary, input
audit, module summary, and interface summary.
...
The function exports module, interface, and null-distribution figures. The null
figure is skipped when the null model was not run.
...
A gene can occur more than once because multi-module assignment is required to
represent interfaces and Level 3B overlap.

## `fruitArchitecture_example_counts.csv`

Synthetic integer counts. The first column is `gene_id`; the remaining columns
are six samples:
